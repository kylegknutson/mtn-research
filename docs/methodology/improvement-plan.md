...
- [x] **Retroactive regional-map backfill** *(noted 2026-06-07, done 2026-06-08)* — ran `sync_to_regional.py` (dedupe ON) across all 14 pre-existing reports. Result: 13 were already fully present in their regional maps ("0 new"); only `pt_13155_group` had a gap (**+11 tracks** into San Juan `06AR6BF`). All reports' tracks are now in their regional maps. (`dolores_middle_peak` skipped — no local tracks.)
