...
<!-- CLIMBERS_START -->
**Other climbers:** Emily Sharpe — not yet · Shawn D Keil — 1 of 2 (Broken Hand Pk)
<!-- CLIMBERS_END -->
...
**Why these two together:** this is a **standard combination**, not a forced pairing. Of the trip reports that hit either peak, the majority do both in one outing:
...
3. **Broken Hand Peak (13,575', Class 3):** from the pass, take the standard Class 3 line to the summit (NW/west side off the pass — *not* the south ridge, which is a hard technical variant; see *Other considerations*)
...
> **Honest caveat:** the cleanest *easy-class* way to bag both may actually be from **Cottonwood Creek** (full reasoning in *Other considerations*).
...
- **Permits / access:** Sangre de Cristo Wilderness — no permits required. Rio Grande / San Isabel National Forest — no fee. South Colony Road: 2WD to the lower lot; high-clearance 4WD for the upper ~2.5 mi (drivable in a careful stock AWD per Marmot72)
- **Cell coverage:** 14ers.com community DB has 0 reports for both Broken Hand Peak and Crestolita (consistent with the broader Sangre de Cristo gap). **South Colony TH / approach:** likely **dead** — the trailhead and lower basin sit deep on the east side of the range, topographically shadowed from the San Luis Valley towers. **Broken Hand Pass / summits:** possible signal on the high points — line-of-sight west toward the San Luis Valley (Crestone/Moffat); don't count on it. Carry an **InReach / satellite messenger** — community data is nonexistent here and the terrain is committing (especially the technical traverse, which has no bail).
...
The cleanest *easy-class* way to bag both may actually be from **Cottonwood Creek**, where
each peak has a Class 2 south side and you skip the Broken Hand Pass elevation — at the
cost of a longer, rougher approach (per Marmot72). From South Colony you get a shorter
approach but the link between the two involves descending/contouring, not a simple ridge
walk. Cottonwood makes sense mainly if:
